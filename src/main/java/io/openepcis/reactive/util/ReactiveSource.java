...
/**
 * Abstraction for reactive input sources in document processing pipelines.
 *
 ...
 * ReactiveSource source = ReactiveSource.fromPublisher(httpBody);
 *
 * // From Mutiny Multi
 * Multi<ByteBuffer> multi = ...;
 * ReactiveSource source = ReactiveSource.fromMulti(multi);
 *
 * // From Netty ByteBuf
 * Flow.Publisher<ByteBuf> nettyPublisher = ...;
 * ReactiveSource source = ReactiveSource.fromNettyPublisher(nettyPublisher);
 *
 * // From InputStream (backward compatibility)
 * try (InputStream is = new FileInputStream("document.xml")) {
 *     ReactiveSource source = ReactiveSource.fromInputStream(is);
 *     // Use source...
 * }
 *
 * // From byte array (testing/convenience)
 * byte[] data = Files.readAllBytes(path);
 * ReactiveSource source = ReactiveSource.fromBytes(data);
 * }</pre>
 *
 * @see NettyBufferSupport
 * @see ByteBufferChunker
 */
public final class ReactiveSource {

  private static final Logger LOG = System.getLogger(ReactiveSource.class.getName());
   ...
   * @return new ReactiveSource
   * @throws NullPointerException if publisher is null
   */
  public static ReactiveSource fromPublisher(Flow.Publisher<ByteBuffer> publisher) {
    return new ReactiveSource(publisher, null, false, null);
  }
   ...
   * @throws NullPointerException if publisher is null
   */
  public static ReactiveSource fromPublisher(
      Flow.Publisher<ByteBuffer> publisher,
      Callable<Flow.Publisher<ByteBuffer>> retrySource) {
    return new ReactiveSource(publisher, retrySource, false, null);
  }
   ...
   */
  public static ReactiveSource fromMulti(Multi<ByteBuffer> multi) {
    Objects.requireNonNull(multi, "Multi cannot be null");
    return new ReactiveSource(multi, null, false, null);
  }
   ...
   */
  public static ReactiveSource fromNettyPublisher(
    ...
    return new ReactiveSource(converted, null, true, publisher);
  }
   ...
   * @throws IllegalStateException if Netty is not available
   */
  public static ReactiveSource fromNettyMulti(Multi<io.netty.buffer.ByteBuf> multi) {
    Objects.requireNonNull(multi, "Multi cannot be null");
    NettyBufferSupport.checkAvailable();
    ...
    return new ReactiveSource(converted, null, true, multi);
  }
   ...
   */
  public static ReactiveSource fromInputStream(InputStream inputStream) {
   ...
   */
  public static ReactiveSource fromInputStream(InputStream inputStream, int bufferSize) {
   ...
   * @throws NullPointerException if inputStream is null
   */
  public static ReactiveSource fromInputStream(
   ...
   */
  public static ReactiveSource fromBytes(byte[] bytes) {
    Objects.requireNonNull(bytes, "Bytes array cannot be null");
    return new ReactiveSource(createBytesPublisher(bytes), null, false, null);
  }
   ...
   * @throws NullPointerException if bytes is null
   */
  public static ReactiveSource fromBytes(byte[] bytes, Callable<byte[]> retryBytes) {
    Objects.requireNonNull(bytes, "Bytes array cannot be null");
    Callable<Flow.Publisher<ByteBuffer>> retryPublisher = retryBytes == null ? null
        : () -> createBytesPublisher(retryBytes.call());
    return new ReactiveSource(createBytesPublisher(bytes), retryPublisher, false, null);
  }
